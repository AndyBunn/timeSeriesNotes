...
```{r,echo=FALSE}
set.seed(872)
#knitr::purl("06TimeSeries-Forecast.Rmd")
```
...
forecast_df <- tibble(
  time = (n + 1):(n + 3),
  value = c(y_hat_1, y_hat_2, y_hat_3),
  se = c(se_1, se_2, se_3),
  type = "Forecast"
)
...
ggplot(plot_df, aes(x = time, y = value, color = type)) +
  ...
  geom_line(data = filter(plot_df, type == "Observed")) +
  geom_point(size = 2) +
  geom_errorbar(
    data = forecast_df,
  ...
  labs(
    title = "AR(1) Forecast with Uncertainty",
    y = "y", x = "Time"
  ) +
  theme_minimal() +
...
ch4.hat <- predict(ch4.ar,n.ahead=length(time(ch4.test)))
cor(ch4.hat$pred,ch4.test)
plot(ch4.ts,type="n",ylab="ppb")
abline(v=2016,lty="dotted")
lines(ch4.train,col="darkblue")
lines(ch4.test,col="darkgreen")
lines(ch4.hat$pred,col="darkgreen")
```
...
```{r,echo=FALSE,eval=FALSE}
ch4.hat <- predict(ch4.hw,n.ahead=length(time(ch4.test)))
cor(ch4.hat,ch4.test)
plot(ch4.ts,type="n",ylab="ppb")
abline(v=2016,lty="dotted")
lines(ch4.train,col="darkblue")
lines(ch4.test,col="darkgreen")
lines(ch4.hat,col="darkgreen")
```
...
```{r,echo=FALSE,eval=FALSE}
Ccf(pg$producers,pg$grazers,lag.max = 50) 
# lag at 40
pg$producersLag40 <- dplyr::lag(pg$producers,40)
# note correlation for grazers and producersLag40
cor(pg,use="complete.obs")
# here is a model
lmGrazers <- lm(grazers~producersLag40,data=pg)
summary(lmGrazers)

pg2 <- readRDS("PhytoExp2.rds")
tmpGrazers <- predict(lmGrazers,
                      newdata = data.frame(producersLag40=pg2$producers))
# note this is the same as
tmpGrazers <- lmGrazers$coefficients[1] + lmGrazers$coefficients[2] * pg2$producers
# insert the lag
pg2$grazersHat <- dplyr::lag(tmpGrazers,40)
pg2Long <- pg2 %>% pivot_longer(cols=-1)
# and make a nice ggplot
ggplot(pg2Long,aes(x=day,y=value,color=name)) +
  geom_line() +
  geom_smooth(se = FALSE) +
  labs(y="Relative biomass",
       x="Number of days from start of experiment")
# compare to first ccf plot
Ccf(pg2$producers,pg2$grazers,lag.max = 50) 
```
