...
GLS is OLS with $\boldsymbol{\Sigma}^{-1}$ inserted as a weighting matrix. When $\boldsymbol{\Sigma}$ is just $\sigma^2 \mathbf{I}$ (equal variances, no correlation), the two formulas are identical -- OLS is a special case of GLS. If the matrix notation here is unfamiliar, the [OLS via Algebra and Matrices aside](07xAsideOLS_Algebric_Matrix.html) walks through where both formulas come from and why they are structured the way they are.
