...
n <- 1000
tm <- 1:n
signal <- sin(2 * pi * tm / 50) + 0.5 * sin(2 * pi * tm / 200) + rnorm(n, sd = 0.5)

# Time-domain data frame
dat <- tibble(time = tm, signal = signal)

# Time-domain plot
p_time <- ggplot(dat, aes(x = time, y = signal)) +
  geom_line(color = "steelblue") +
  labs(title = "Time Domain", x = "Time", y = "Signal") +
  theme_minimal()

# Frequency-domain: periodogram
spec_obj <- spectrum(signal, plot = FALSE)
spec_df <- tibble(freq = spec_obj$freq, spec = spec_obj$spec)

# Frequency-domain plot
p_freq <- ggplot(spec_df, aes(x = freq, y = spec)) +
  geom_line(color = "firebrick") +
  labs(title = "Frequency Domain", x = "Frequency (1 / time unit)", y = "Spectral Density") +
  theme_minimal()
# Show plots side by side
library(cowplot)
plot_grid(p_time,p_freq,ncol = 1)
```
...
tm <- 1:n
white <- rnorm(n)
ar1 <- c(arima.sim(model = list(ar=0.6),n=n))
# Create the random walk by cumulative sum
randomWalk <- cumsum(white)


plotSpec <- function(y,span = NULL){
  ySpec <- spectrum(y, span=span, plot= FALSE)
  tm <- 1:length(y)
  # Time-domain data frame
  signal_df <- tibble(time = tm, signal = y)
  # Frequency-domain data frame
  spec_df <- tibble(freq = ySpec$freq, spec = ySpec$spec)
  
  # Time-domain plot
  p_time <- ggplot(signal_df, aes(x = time, y = signal)) +
    geom_line(color = "steelblue") +
    labs(title = "Time Domain", x = "Time", y = "Signal") +
    theme_minimal()
...
jul65N.spec <- spectrum(jul65N$W.per.m2, span=5, plot= FALSE)
plot(jul65N.spec,log="no", type="h", xlab="Frequency (cycles / kyr)",
     ylab="Spectral density")
abline(v=1/19,col="red")
#1/19

foo <- ts(jul65N$W.per.m2,start=-5000,freq=1e-3)
plot(foo)
foo.spec <- spectrum(foo, span=5, plot= FALSE)
plot(foo.spec,log="no", type="h", xlab="Frequency (cycles / yr)", 
     ylab="Spectral density")
abline(v=1/19*1e-3,col="red")
